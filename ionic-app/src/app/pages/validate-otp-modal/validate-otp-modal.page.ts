...
@Component({
  ...
  templateUrl: './validate-otp-modal.page.html',
  styleUrls: ['./validate-otp-modal.page.scss'],
})
export class ValidateOtpModalPage implements OnInit {
  ...
  getToastMessage(contact: string): string {
    let result = /^\d+$/.test(contact.substring(1));
    if (result){
      return 'COMMON.otpSendBySMSForRestPassword';
    }
    else {return 'COMMON.otpSendByEmailForRestPassword'};
  }
    ...
    this.authService.getOtp(this.email).subscribe(
      response => {
        this.toastService.showInfoToast(
          this.translateService.instant(this.getToastMessage(this.email)));
      },
      error => {
